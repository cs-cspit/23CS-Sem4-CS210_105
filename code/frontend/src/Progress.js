...
import { getAuthToken, isAuthenticated, clearAuthData, getUserRole } from './utils/storage';
...
function DashboardContent({setIsLoggedIn,navigate,user }) {
  ...
  useEffect(() => {
    axios.get(`${API_BASE_URL}/user/courses`, {headers:{'Authorization': getAuthToken()}})
      .then((resp) => {   // if no error
