...
function DashboardContent({setIsLoggedIn,navigate,user }) {
  ...
  const handleSave = () => {
    axios.put(`${API_BASE_URL}/user/${user.user._id}`,{
      username : username,
      email : email,
      phone : phone
  ...
  useEffect(() => {
    axios.get(`${API_BASE_URL}/user/${user.user._id}`,{headers:{'Authorization':get('token')}})
    .then((resp)=>{   // if no error
  ...
  const handleSubmitChangePassword = async (e) => {
    ...
    axios.put(`${API_BASE_URL}/user/change/password`,{
